...
/*
 * Внешний вид станции (большое окно «Радио»): темы, акцент, дисплей (стиль, цвет цифр, фон, узор,
 * свечение), фон приложения, шрифты, сброс. Настройки — в localStorage, свои картинки — в IndexedDB.
 * Цвета уходят в CSS-переменные окна (--amber, --vfd, --page, --mono…), режимы — атрибутами на <html>
 * для look-station.css. Снаружи: window.StationLook.open().
 */
(() => {
  const KEY = 'station.pc.look';
  const root = document.documentElement;

  const DEFAULTS = {
    accent: '#ffb547', digits: '#6ef2d0', disp: 'classic', lcd: '#c6d6b2', pattern: 'none',
  ...
  const COLORS = {
    ...
    digits: ['#6ef2d0', '#5ef0c4', '#ffb000', '#7fdcff', '#ff6b6b', '#39ff88', '#f2f2f2', '#d5b8ff', '#c8ff00'],
  ...
  const BACKGROUNDS = {
    dark: { name: 'Тёмный', css: 'radial-gradient(1200px 600px at 50% -10%, #1b1c22 0%, #0b0c0f 60%), #0b0c0f' },
  ...
  const THEMES = {
    classic: { name: 'Классика', sw: ['#ffb547', '#6ef2d0'], set: {} },
  ...
  function db() {
    return new Promise((resolve, reject) => {
      const req = indexedDB.open('station-pc-look', 1);
  ...
  /* ───────── Применение (в переменные окна «Радио») ───────── */
  ...
  function apply() {
    const v = {};
    v['--amber'] = look.accent;
    v['--amber-soft'] = rgba(look.accent, 0.14);

    const light = look.disp === 'lcd';
    const digits = light ? '#152013' : look.digits;
    v['--vfd'] = digits;
    v['--vfd-glow'] = rgba(digits, light ? 0.25 : 0.55);
    ...
    if (light) {
      ...
      v['--vfd-ghost'] = shade(look.lcd, -0.4);
    } else if (look.disp === 'oled') {
      base = '#000000';
      v['--disp-bg'] = '#000';
      v['--disp-edge'] = mix('#000000', digits, 0.2);
      v['--vfd-ghost'] = mix('#000000', digits, 0.32);
    } else {
      base = mix('#050807', digits, 0.08);
      v['--disp-bg'] = `radial-gradient(120% 100% at 50% 0%, ${mix('#0b1a17', digits, 0.1)}, ${mix('#050b0a', digits, 0.03)} 70%)`;
      v['--disp-edge'] = mix('#0b0c0f', digits, 0.22);
      v['--vfd-ghost'] = mix(base, digits, 0.32);
    }
    v['--disp-base'] = base;
    ...
    v['--mono'] = df?.family || "'JetBrains Mono', ui-monospace, 'Cascadia Mono', Consolas, monospace";
    const uf = FONTS[look.fontUi];
    if (uf?.family) v['--sans'] = uf.family;
    else v['--sans'] = 'system-ui, "Segoe UI", Roboto, sans-serif';

    // Фон приложения
    let appBg;
    if (look.bg === 'color') appBg = look.bgColor;
    else if (look.bg === 'image' && urls.bg) {
      const dim = look.bgDim / 100;
      appBg = `linear-gradient(rgba(0, 0, 0, ${dim}), rgba(0, 0, 0, ${dim})), url("${urls.bg}") center / cover no-repeat fixed, #000`;
    } else appBg = (BACKGROUNDS[look.bg] || BACKGROUNDS.dark).css;
    ...
    // Дисплей: перекрываем жёстко заданный фон переменной, цвет цифр — через --vfd
    css += ` .display { background: var(--disp-bg) !important; color: var(--vfd-ghost); }`;
    css += ` .radio[data-power="on"] .display { color: var(--vfd); }`;
    css += ` body { background: ${appBg} !important; }`;
  ...
  function imageButtons(name, onPick, maxSide, onDrop) {
    const row = el('div', 'lk__row');
    const pick = el('button', 'lk__btn lk__btn--accent', images[name] ? 'Другая картинка' : 'Выбрать файл');
    pick.type = 'button';
