...
/*
 * Сетевые фишки, как у настоящих раций (⚙ на рации → «Канал»):
 *   • Тоновый шумодав (CTCSS): в передачу подмешивается неслышный тон 67–100 Гц (ниже среза 110 Гц,
 *     поэтому старые рации и станция его почти не слышат). На приёме звук открывается только когда в
 ...
 * Всё на этом компьютере, сервер и другие рации менять не нужно. Работает поверх кода рации.
 */
(() => {
  ...
  // Не дать начать передачу на занятом канале. Возвращает true, если передачу надо заблокировать.
  function guardPtt() {
    if (!cfg.bcl) return false;
    if (channelBusy()) {
      blockPtt();
      return true;
    }
  ...
  window.WalkieChannel = { section, cfg, toneStrength, guardPtt };
})();
