...
/*
 * Внешний вид рации: темы, корпус, кнопки, экран, антенна, свечение, шрифты.
 ...
 * Снаружи: window.WalkieLook.open() — редактор.
 */
(() => {
  ...
  const FINISHES = {
    ...
    'skin-woodland': 'Камуфляж лес', 'skin-desert': 'Камуфляж пустыня', 'skin-urban': 'Камуфляж город',
    'skin-arctic': 'Камуфляж арктика', 'skin-purple': 'Камуфляж фиолет', 'skin-red': 'Камуфляж красный',
  };

  // Фотоскины: фактура = готовый рендер корпуса и кнопок (skins/<цвет>_*.png). Живые элементы
  // (экран, кнопки с подписями, боковые, ручка) кладутся в вырезы — см. skins.css. Значение —
  // префикс файлов ассета.
  const SKINS = {
  ...
  function apply() {
    ...
    // Фотоскины камуфляжа: готовые рендеры корпуса, кнопок, боковых, ручки кладутся картинками
    // в CSS-переменные, а skins.css расставляет их по вырезам корпуса. Живые элементы (экран,
    // кнопки клавиатуры, боковые кнопки, ручка) сохраняют работу и анимацию — меняется только вид.
  ...
  // Фон страницы (bg) на ПК не показывается — окно рации прозрачное, поэтому вкладки «Фон» нет
  const TABS = [
    ['themes', 'Темы'], ['body', 'Корпус'], ['keys', 'Кнопки'], ['screen', 'Экран'], ['fonts', 'Шрифты'],
  ];
  let tab = 'themes';
  let panel = null;
  let body = null;
